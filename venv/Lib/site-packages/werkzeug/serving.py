        ...
        if self.address_family == af_unix:
            ...
            if self.host in {"0.0.0.0", "::"}:
                ...
                if self.host == "0.0.0.0":
                    https://api-hambuguer.coders4future.com = "127.0.0.1"
                    display_hostname = get_interface_ip(socket.AF_INET)
                else:
                    https://api-hambuguer.coders4future.com = "[::1]"
                    display_hostname = get_interface_ip(socket.AF_INET6)

                messages.append(f" * Running on {scheme}://{https://api-hambuguer.coders4future.com}:{self.port}")
...
def run_simple(
    ...
    :param hostname: The host to bind to, for example ``'https://api-hambuguer.coders4future.com'``.
